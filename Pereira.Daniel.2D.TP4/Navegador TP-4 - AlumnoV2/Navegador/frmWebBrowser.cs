...
namespace Navegador
{
    public partial class frmWebBrowser : Form
    {
        ...
        private void FinDescarga(string html)
        {
            ...
            { //si devuelve true tiene que invocar al control atravez de un delegado
                FinDescargaCallback d = new FinDescargaCallback(FinDescarga);
                this.Invoke(d, new object[] { html });
            }
            else
            {//si devuelve false llama al control directamente
        ...
        {
            // verificamos que "http://" este al comienzo de la cadena sino lo concatenamos
            if (!txtUrl.Text.StartsWith("http://"))
                txtUrl.Text = "http://" + txtUrl.Text;

            Uri URL = new Uri(txtUrl.Text);
            Descargador desc = new Descargador(URL);
            desc.EvenProg += ProgresoDescarga;
            desc.EvenFinDes += FinDescarga;
            Thread hilo1 = new Thread(desc.IniciarDescarga);
            hilo1.Start();
        ...
        {
            frmHistorial fHistorial = new frmHistorial();
            fHistorial.Show();
        }
    }
}
